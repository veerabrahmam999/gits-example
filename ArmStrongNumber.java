/*This class provides a functionality to find whether the number is Arm-Strong or not
 */
...
public class ArmStrongNumber {
    public static void main(String[] args) {
       ...
       boolean result = isArmStrong(number,numberLength, number);
       if(result==true){
           System.out.println(number+" is an Arm-Strong Number");
       }
       else{
    ...
    private static boolean isArmStrong(int number,int numberLength,int tempVariable ){
        double sum = 0;
        while(number!=0){
            int remaining = number%10;
            sum = sum+Math.pow(remaining,numberLength);
            number/=10;
        }
        if(sum==tempVariable){
            return true;
